...
Für die Implementierung der Browser-Anwendung wird auf den aktuellen Entwurf des zukünftigen
HTML5-Standards zurückgegriffen.
...
allem auch die Skriptsprache \acr{js} und die darin verfügbaren Bibliotheken sowie das \acr{dom},
...
Der Einstiegspunkt in das \acr{dom} ist der \texttt{document}-Knoten, welcher in \acr{js}
...
\textit{Silverlight} implementiert hat nun in reinem \acr{html}+\acr{css} zu verwirklichen. Die für
...
Auch \acr{css}3 hat immer noch einige konzeptionelle Einschränkungen, welche die Benutzung
erschweren:
...
angehauchte Sprache wie \acr{js} ungeeignet, da Funktionsdefinitionen durch geschweifte Klammern,
...
\textit{CoffeeScript}\footnote{http://www.coffeescript.org} ist eine neue Skriptsprache mit dem Ziel
...
gebunden werden. CoffeeScript Dateien werden zu optimiertem \acr{js} Code kompiliert, der den
...
Das \acr{http} ist das im Internet verwendete Standardprotokoll zur Übertragung von Daten. (Siehe
...
guten Unterstützung in \acr{js} und den meisten Web-Frameworks auch üblich \acr{json} zur Übertragung
zu nutzen. \cite{ajax}
...
Web-Entwickling. Besonders erwähnenswert ist hierbei auch die \acr{ajax}-Abstraktion. Eine gute
